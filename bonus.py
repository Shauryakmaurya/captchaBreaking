...
class CaptchaClassifier(nn.Module):
    ...
    def forward(self, images, background_color=None):
        ...
        # Condition for the bonus Generation task
        if background_color is not None:
            if background_color == "red":
                x = torch.flip(x, dims=[1])  # Reverse the word if the background is red
...
if __name__ == "__main__":
    ...
    output = model(img, background_color="red")
